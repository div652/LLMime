...
* **Current resolution (interim):** `compiler.parse_table` flattens a Markdown table into valid `unstyled` paragraph rows (cells joined by `|`, header cells bold, inline math/bold preserved). This pastes cleanly and never poisons the fragment. It is *readable*, not a *native* table.
* **Path to native tables:** Implement a separate `database-fragment` generator, used **only when the entire copied selection is a single table** (so the standalone constraint is satisfied). Open questions to resolve by experiment: which of `createdBy`/`updatedBy`/timestamps are required vs. regenerated by Slite on paste, and whether `database-field` is mandatory. See Future Task 1.
...
#### Task 1: Native Tables via `database-fragment` (single-table copies only)
* **Status:** Schema discovered (Section 7). Interim flattening shipped. Native generation not yet built.
* **Plan:** When the copied selection is *only* a Markdown table, emit a `application/x-slite-database-fragment` (and possibly `application/x-slite-database-field`) payload instead of `x-slite-global`: map each Markdown column → a `text` column, each body row → a record, and each cell → a `database-table-cell` in `content` holding an `unstyled` block (reuse the existing inline pipeline so math/bold survive). Mixed content (heading + table) must keep using the flattened fallback because a database can't be embedded inline.
* **Experiments to settle:** copy a *freshly created, all-text* 2×2 Slite table and dump it to see the minimal skeleton; determine whether `createdBy`/`updatedBy`/timestamps are required or regenerated on paste, and whether `database-field` is mandatory.
